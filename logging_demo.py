...
def logging_test(logger):
    ...
    logger.debug("Detected some bug please fix it")
    logger.info("Server is in modify please wait")
    logger.warning("Connection Lost")
    logger.error("TypeError please use ValueError")
    logger.critical("Critical Error: Start menu and Cortana aren't working.")
    level = logging.WARN + 5  # custom log level
    logger.log(level, "This is log error thank you.")
...
def my_config():
    ...
    FORMAT = '{ %(asctime)s | %(name)s | %(levelname)s } : %(message)s'
    logging.basicConfig(format=FORMAT, level=logging.NOTSET, filemode='w', filename="log_text")
